<?php
require_once (DIR_SYSTEM.'/library/tmd/paytrail/autoload.php');
class ControllerExtensionPaymentPaytrail extends Controller {
   ...
   $body = json_encode(        
        ...
        'redirectUrls' => [
            'success' =>$this->url->link('extension/payment/paytrail/callback', '', true),
	...
	if ($responseHmac !== $response->getHeader('signature')[0]) {
		...
		return $this->load->view('extension/payment/paytrail', $data);
	}
